...
# 口语缩写 → 正式课名
COURSE_ALIAS = {
    '高数': '高等数学', '线代': '线性代数', '高代': '高等代数', '大物': '大学物理',
    '概率': '概率论', '离散': '离散数学', '数电': '数字电路', '模电': '模拟电路',
    '计组': '计算机组成原理', '操统': '操作系统', '编原': '编译原理', '大英': '大学英语',
}
COURSE_SUBSTR_BLOCK = (
...
def _alias_hits(line):
    """行内出现的课程简称 → 正式名（强信号：作者在点名这门课）。

    知识库正文大量用简称（"高数加线代是后面所有课的地基"）。不认简称的话，
    这些课在本年段里抽不到，只能从别处的课程表落进"未归年池"，回填时被挤到错误的年级
    ——实测计算机专业的高等数学/线性代数原先被排到了大三。
    """
    return [(1, full) for ab, full in COURSE_ALIAS.items() if ab in line]


def _line_candidates(line):
    """一行文本 → [(tier, 课程名)]；tier1=强信号（书名号/加粗/小标题/冒号引导/顿号列举），tier2=弱信号。"""
    out = list(_alias_hits(line))
