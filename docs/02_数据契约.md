...
> v1.0 · 本文档与 /src/contracts 一一对应。改动需项目负责人确认，Claude Code 不得自行扩展。

## 1. PlayerState（唯一状态源，Zustand store + localStorage 持久化）
...
## 4. 存档
- key：`unisim_save_v1`；写入时机：每次 onComplete/onEscape 与学期流转后
